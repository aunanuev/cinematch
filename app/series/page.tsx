...
import { subscribeToSeries } from "@/lib/firebase/firestore";
...
import { AddSeriesFab } from "@/components/dashboard/AddSeriesFab";
import { AddSeriesModal } from "@/components/dashboard/AddSeriesModal";
...
export default function SeriesDashboard() {
    ...
    useEffect(() => {
        if (user) {
            const unsubscribe = subscribeToSeries(user.uid, (data) => {
                setSeriesList(data);
            });
            return () => unsubscribe();
        }
    ...
    const filtered = seriesList.filter(item => {
        ...
        if (filter === "pending" && item.watched) return false;
        if (searchQuery && !item.title.toLowerCase().includes(searchQuery.toLowerCase())) return false;
        if (selectedTag && !(item.vibeTags || []).includes(selectedTag)) return false;
        return true;
    });
    ...
    const genreMap = useMemo(() => {
        ...
        for (const item of filtered) {
            const primaryGenre = item.genres && item.genres.length > 0 ? item.genres[0] : "Uncategorized";
            if (!map.has(primaryGenre)) map.set(primaryGenre, []);
            map.get(primaryGenre)!.push(item);
        }
    ...
    const watchedCount = seriesList.filter(m => m.watched).length;
    const pendingCount = seriesList.filter(m => !m.watched).length;

    const allVibeTags = useMemo(() => {
        const tagSet = new Set<string>();
        seriesList.forEach(m => (m.vibeTags || []).forEach(t => tagSet.add(t)));
        return [...tagSet].sort();
    }, [seriesList]);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-[#0B1220] via-[#0F172A] to-[#020617] text-gray-100 font-sans">
            {/* Ambient glows */}
            <div className="fixed top-0 left-0 w-72 h-72 bg-purple-900/20 rounded-full blur-[100px] pointer-events-none" />
            <div className="fixed bottom-0 right-0 w-72 h-72 bg-indigo-900/15 rounded-full blur-[100px] pointer-events-none" />
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center bg-slate-900/60 border border-white/5 rounded-lg p-0.5">
                                <button
                                    ...
                                    className={`p-1.5 rounded-md transition-all duration-200 ${viewMode === "grid" ? "bg-purple-600 text-white" : "text-gray-500"
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`p-1.5 rounded-md transition-all duration-200 ${viewMode === "list" ? "bg-purple-600 text-white" : "text-gray-500"
                                        }`}
                                >
                    ...
                    {filtered.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                            {searchQuery || selectedTag ? (
                                ...
                                <>
                                    <p className="text-lg text-gray-500 mb-1">No series here yet.</p>
                    ...
                    {filtered.length > 0 && (
                        groupByGenre ? (
                            <div className="space-y-8">
                                {[...genreMap.entries()].map(([genre, genreItems]) => (
                                    <GenreSection
                                        key={genre}
                                        genre={genre}
                                        items={genreItems}
                            ...
                            <MediaGrid items={filtered} mediaType="series" viewMode={viewMode} collectionTags={allVibeTags} />
                        )
                    )}
                </main>
            </div>
